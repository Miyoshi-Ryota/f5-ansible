...
from ansible_collections.ryotamiyoshi.f5_modules.plugins.modules.bigip_provision_async import (
...
class TestManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher1.start()
        self.p2 = patch('ansible_collections.ryotamiyoshi.f5_modules.plugins.modules.bigip_provision_async.tmos_version')
        self.p3 = patch('ansible_collections.ryotamiyoshi.f5_modules.plugins.modules.bigip_provision_async.send_teem')
        self.m2 = self.p2.start()
        self.m2.return_value = '15.1.0'
    ...
    def test_start_provision_one_module_default_level(self, *args):
        ...
        assert "Device will start provisioning requested module: gtm" in results['message']

    def test_check_for_provision_progress_one_module_default_level(self, *args):
        # Configure the arguments that would be sent to the Ansible module
        set_module_args(dict(
            module='gtm',
            check_status=True,
        ...
        mm._is_mprov_running_on_device = Mock(side_effect=[True, True, False, False, False])

        results = mm.exec_module()

        assert results['changed'] is True
        assert "Device has finished provisioning the requested module." in results['message']
