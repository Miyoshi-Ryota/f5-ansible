...
```bash
ansible-galaxy collection install ryotamiyoshi.f5_modules
```
...
```bash
ansible-galaxy collection install ryotamiyoshi.f5_modules -p collections/
```

To specify the version of the collection to install, include it at the end of the collection with `:==1.0.0`:
```bash
ansible-galaxy collection install ryotamiyoshi.f5_modules:==1.0.0
```
...
- name: Using Collections
  ...
  tasks:
    - ryotamiyoshi.f5_modules.bigip_pool:
        name: my-pool
        ....
...
ansible-galaxy collection install git+https://github.com/ryotamiyoshi/f5-ansible-bigip.git#ansible_collections/ryotamiyoshi/f5_bigip
```
...
[dailybuild]: https://f5-ansible.s3.amazonaws.com/collections/ryotamiyoshi-f5_modules-devel.tar.gz
